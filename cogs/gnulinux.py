...
class gnulinux(commands.Cog):
    ...
    async def deepin(self, ctx): # Deepin
        channel = ctx.message.channel
        await ctx.send("bot: It Will Be Done - Command Result Below")
        deepin1=discord.Embed(title="Deepin", url="https://www.deepin.org", description="Deepin (stylized as deepin; formerly known as Linux Deepin and Hiweed Linux) is a Linux distribution based on Debian's stable branch. It features DDE, the Deepin Desktop Environment, built on Qt and available for various distributions like Manjaro, Arch or Fedora. As of version 15.10 it also uses dde-kwin, a set of patches for KDE Plasma's window manager. In 2019, Huawei started to ship Linux laptops pre-installed with deepin. ", color=0x1793d1)
